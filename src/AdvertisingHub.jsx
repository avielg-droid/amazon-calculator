...
function validateAdvertisingReport(parsed) {
  ...
  const date = findHeader(headers, ["date", "start date"]);
  ...
  if (!date) return "Choose Date as the time dimension so Danuly can compare periods.";
...
function columnsFor(parsed) {
  const headers = parsed.headers;
  return {
    date: findHeader(headers, ["date", "start date"]),
...
function marketMatches(row, countryHeader, market) {
  ...
  return MARKETS[market].aliases.includes(normalize(row[countryHeader]));
}
...
function analyzeAdvertising(parsed, month, market, targetRoas) {
  ...
  return { current, previous, campaigns, currentRows, bounds };
}
...
export default function AdvertisingHub({ onBack }) {
  ...
  const analysis = useMemo(function () { return advertising && vendorSales ? analyzeAdvertising(advertising, month, market, Number(targetRoas) || 4) : null; }, [advertising, vendorSales, month, market, targetRoas]);
