...
function getSalesHeader(headers, basis) {
  ...
  const dispatched = ["dispatched revenue", "shipped revenue"];
...
function validateSalesReport(parsed, basis) {
  ...
  if (!getSalesHeader(parsed.headers, basis)) {
    return basis === "ordered"
      ...
      : "Dispatched retail revenue was not found. Export the Sales report with dispatched revenue, or change the sales basis.";
  }
  return null;
}
...
export default function SellOutControlTower({ onBack, navigation }) {
      ...
      <div style={{ maxWidth: 1040, margin: "0 auto" }}>
        ...
        <div style={{ margin: "18px 0 20px", display: "flex", flexWrap: "wrap", gap: 16, justifyContent: "space-between", alignItems: "flex-end" }}>
          <div style={{ maxWidth: 670 }}>
            <span style={{ color: C.sky, fontSize: 10, fontWeight: 800, letterSpacing: "0.07em" }}>DIRECTOR SELL-OUT CONTROL TOWER</span>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 10, marginBottom: 14 }}>
            ...
            <label style={{ fontSize: 10, fontWeight: 700, color: C.muted }}>
              ...
              <select value={salesBasis} onChange={function (event) { setSalesBasis(event.target.value); setUploads([]); setSaved(false); }} style={inputStyle({ marginTop: 5 })}>
                <option value="ordered">Ordered revenue (recommended)</option>
                <option value="dispatched">Dispatched retail revenue</option>
              </select>
            </label>
          </div>
