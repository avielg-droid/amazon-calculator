...
const C = {
  emerald: "#10b981", emeraldDim: "#10b98118",
  cyan: "#06b6d4",    cyanDim: "#06b6d418",
  orange: "#f97316",  orangeDim: "#f9731618",
  rose: "#f43f5e",    roseDim: "#f43f5e12",
  violet: "#8b5cf6",  violetDim: "#8b5cf618",
  amber: "#f59e0b",   amberDim: "#f59e0b18",
  blue: "#3b82f6",    blueDim: "#3b82f618",
  light: "#f1f5f9",
  s3: "#cbd5e1", s4: "#94a3b8", s5: "#64748b", s6: "#475569",
  s7: "#334155", s8: "#1e293b", s85: "#162032", s9: "#0f172a", s95: "#080f1a",
};
...
const MONO = { fontFamily: "ui-monospace,'JetBrains Mono',monospace", fontVariantNumeric: "tabular-nums" };
const SANS = { fontFamily: "ui-sans-serif, system-ui, sans-serif" };
const CARD = { background: C.s9, border: `1px solid ${C.s8}`, borderRadius: 14, padding: "20px 22px" };
const CARD_INSET = { background: C.s95, border: `1px solid ${C.s85}`, borderRadius: 10, padding: "14px 16px" };
const LABEL = { fontSize: 10, fontWeight: 700, letterSpacing: "0.09em", textTransform: "uppercase", color: C.s5, marginBottom: 4, display: "block" };
const DIVIDER = { height: 1, background: C.s8, margin: "12px 0" };
const ROW = { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${C.s85}` };

function InputField({ label, name, value, onChange, prefix, suffix, highlight, disabled, dimNote, tooltip, error }) {
  const [focused, setFocused] = useState(false);
  const [tipVisible, setTipVisible] = useState(false);
  const accent = highlight || C.emerald;
  return (
    <div style={{ marginBottom: 11, opacity: disabled ? 0.45 : 1 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 5 }}>
        ...
        {tooltip && (
          <span style={{ position: "relative", display: "inline-flex", alignItems: "center", cursor: "help" }}
            ...
            <Info size={11} color={C.s6} />
            {tipVisible && (
              <div style={{ position: "absolute", left: 18, top: -4, zIndex: 50, background: C.s7, border: `1px solid ${C.s6}`, borderRadius: 8, padding: "9px 11px", width: 210, fontSize: 11, color: C.s3, lineHeight: 1.55, boxShadow: "0 8px 24px #00000070", pointerEvents: "none" }}>{tooltip}</div>
            )}
          </span>
        )}
      </div>
      ...
      <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
        {prefix && <span style={{ position: "absolute", left: 10, fontSize: 12, color: C.s5, pointerEvents: "none", zIndex: 1 }}>{prefix}</span>}
        <input type="number" name={name} value={value} onChange={onChange} disabled={disabled}
          onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
          style={{ width: "100%", background: C.s95, border: `1px solid ${error ? C.rose : focused ? accent : C.s8}`, borderRadius: 8, padding: `9px ${suffix ? 30 : 12}px 9px ${prefix ? 26 : 12}px`, fontSize: 13, color: C.light, outline: "none", boxSizing: "border-box", transition: "border-color 0.15s, box-shadow 0.15s", boxShadow: focused ? `0 0 0 3px ${accent}22` : "none", cursor: disabled ? "not-allowed" : "auto", ...MONO }}
        />
        {suffix && <span style={{ position: "absolute", right: 10, fontSize: 11, color: C.s5, pointerEvents: "none" }}>{suffix}</span>}
      </div>
      {error && <p style={{ fontSize: 11, color: C.rose, marginTop: 4, marginBottom: 0 }}>{error}</p>}
    </div>
  );
}
...
function StatCard({ label, value, numericValue, thresholds, color, big }) {
  ...
  if (numericValue !== undefined) {
    if (thresholds) { const [lo, hi] = thresholds; col = numericValue >= hi ? C.emerald : numericValue >= lo ? C.amber : C.rose; }
    else col = numericValue >= 0 ? C.emerald : C.rose;
  }
  return (
    <div style={{ background: big ? `${col}0e` : C.s9, border: `1px solid ${big ? col+"35" : C.s8}`, borderRadius: 12, padding: big ? "18px 20px" : "14px 16px", transition: "border-color 0.2s" }}>
    ...
    <div style={{ ...CARD, flex: 1, minWidth: 200 }}>
      ...
      <ResponsiveContainer width="100%" height={170}>
        <PieChart>
          ...
          <ReTooltip formatter={(v, n) => [`$${v.toFixed(2)}`, n]} contentStyle={{ background: C.s7, border: `1px solid ${C.s6}`, borderRadius: 8, fontSize: 12, color: C.light }} />
        </PieChart>
      </ResponsiveContainer>
      <div style={{ display: "flex", flexWrap: "wrap", gap: "5px 12px", marginTop: 6 }}>
        {data.map((d, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: C.s4 }}>
...
function ProgressBar({ value, max = 100, color }) {
  ...
  return (
    <div style={{ height: 5, background: C.s8, borderRadius: 99, overflow: "hidden" }}>
...
function WaterfallBar({ label, value, total, color, isTotal, note }) {
  ...
  return (
    <div style={{ marginBottom: 1 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: isTotal ? "9px 10px" : "6px 10px", borderRadius: 8, background: isTotal ? `${color}0a` : "transparent" }}>
        <span style={{ fontSize: 11, color: isTotal ? C.s3 : C.s4, width: 130, flexShrink: 0, textAlign: "right", fontWeight: isTotal ? 600 : 400 }}>{label}</span>
        <div style={{ flex: 1, height: isTotal ? 7 : 5, background: C.s85, borderRadius: 99, overflow: "hidden" }}>
        ...
        <span style={{ fontSize: 10, color: C.s6, ...MONO, width: 40, textAlign: "right", flexShrink: 0 }}>
...
function InsightCard({ type, title, desc, icon: Icon }) {
  const cfg = {
    danger: { bg: C.roseDim, border: `${C.rose}28`, text: C.rose },
    warning: { bg: C.orangeDim, border: `${C.orange}28`, text: C.orange },
    success: { bg: C.emeraldDim, border: `${C.emerald}28`, text: C.emerald },
    info: { bg: C.cyanDim, border: `${C.cyan}28`, text: C.cyan },
  }[type];
  return (
    <div style={{ background: cfg.bg, border: `1px solid ${cfg.border}`, borderRadius: 10, padding: "13px 15px", display: "flex", gap: 12, alignItems: "flex-start" }}>
      <div style={{ color: cfg.text, flexShrink: 0, marginTop: 1 }}>{Icon ? <Icon size={15} /> : <AlertTriangle size={15} />}</div>
      <div>
        <p style={{ fontSize: 12, fontWeight: 700, color: cfg.text, margin: "0 0 3px" }}>{title}</p>
        <p style={{ fontSize: 11, color: C.s4, margin: 0, lineHeight: 1.55 }}>{desc}</p>
      </div>
    </div>
  );
}
...
function TopBar({ activeTool, onSwitch }) {
  ...
  return (
    <div style={{ background: C.s95, borderBottom: `1px solid ${C.s8}`, padding: "0 20px", height: 52, display: "flex", alignItems: "center", gap: 16, position: "sticky", top: 0, zIndex: 40 }}>
      <button onClick={() => onSwitch("home")} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex", alignItems: "center", gap: 8 }}>
        <div style={{ width: 28, height: 28, borderRadius: 7, background: "linear-gradient(135deg,#10b981,#06b6d4)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <BarChart3 size={14} color="#fff" strokeWidth={2.5} />
        </div>
        <span style={{ fontSize: 14, fontWeight: 800, letterSpacing: "-0.02em", color: C.light }}>Danuly</span>
      </button>
      <div style={{ width: 1, height: 20, background: C.s8 }} />
      <span style={{ fontSize: 12, color: C.s5, fontWeight: 500 }}>{activeTool === "calculator" ? "Profit Calculator" : "PPC Lab"}</span>
      <div style={{ marginLeft: "auto", display: "flex", gap: 3, background: C.s9, border: `1px solid ${C.s8}`, borderRadius: 9, padding: "3px" }}>
        {[{ id: "ppc", label: "PPC Lab", accent: C.violet }, { id: "calculator", label: "Profit Calc", accent: C.emerald }].map(({ id, label, accent }) => (
          <button key={id} onClick={() => onSwitch(id)} onMouseEnter={() => setHovered(id)} onMouseLeave={() => setHovered(null)}
            style={{ padding: "6px 14px", borderRadius: 7, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, transition: "all 0.15s", outline: "none", background: activeTool === id ? accent : hovered === id ? C.s7 : "transparent", color: activeTool === id ? "#fff" : hovered === id ? C.light : C.s5 }}
          >{label}</button>
        ))}
      </div>
    </div>
...
function HomeScreen({ onSelect }) {
  const [hovered, setHovered] = useState(null);
  const tools = [
    { id: "ppc", accent: C.violet, icon: BarChart3, title: "PPC Lab", desc: "Upload your Search Term Report or Search Query Performance data. Find negative keyword candidates, harvest opportunities, and market insights — with export-ready CSVs.", badge: "Search Term & SQP" },
    { id: "calculator", accent: C.emerald, icon: DollarSign, title: "Profit Calculator", desc: "Full unit economics simulator for Amazon FBA and DTC channels. P&L waterfall, cash flow analysis, pricing tools, and AI-driven margin insights.", badge: "FBA + DTC · US & Intl" },
  ];
  return (
    <div style={{ minHeight: "100vh", background: C.s95, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 20px", ...SANS }}>
      <div style={{ textAlign: "center", marginBottom: 56 }}>
        <div style={{ width: 52, height: 52, borderRadius: 14, background: "linear-gradient(135deg,#10b981,#06b6d4)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px", boxShadow: "0 0 0 1px #10b98130, 0 8px 32px #10b98120" }}>
          <BarChart3 size={24} color="#fff" strokeWidth={2} />
        </div>
        <h1 style={{ fontSize: 36, fontWeight: 800, letterSpacing: "-0.03em", margin: "0 0 8px", color: C.light }}>Danuly</h1>
        <p style={{ fontSize: 14, color: C.s5, margin: 0 }}>Amazon seller tools · free & browser-based</p>
      </div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 16, justifyContent: "center", maxWidth: 820, width: "100%" }}>
        {tools.map(({ id, accent, icon: Icon, title, desc, badge }) => {
          const isHov = hovered === id;
          return (
            <div key={id} onClick={() => onSelect(id)} onMouseEnter={() => setHovered(id)} onMouseLeave={() => setHovered(null)}
              style={{ flex: "1 1 300px", maxWidth: 380, background: C.s9, border: `1px solid ${isHov ? accent+"60" : C.s8}`, borderRadius: 16, padding: "28px 26px", cursor: "pointer", transition: "all 0.2s", boxShadow: isHov ? `0 12px 40px ${accent}18` : "0 2px 12px #00000030", transform: isHov ? "translateY(-3px)" : "none", display: "flex", flexDirection: "column", gap: 18 }}
            >
              <div style={{ width: 44, height: 44, background: `${accent}15`, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", border: `1px solid ${accent}25` }}>
                <Icon size={20} color={accent} strokeWidth={2} />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  <span style={{ fontSize: 17, fontWeight: 700, color: C.light }}>{title}</span>
                  <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: accent, background: `${accent}15`, padding: "3px 7px", borderRadius: 5, border: `1px solid ${accent}25` }}>{badge}</span>
                </div>
                <p style={{ fontSize: 13, color: C.s4, margin: 0, lineHeight: 1.65 }}>{desc}</p>
              </div>
              <button style={{ padding: "10px 18px", borderRadius: 9, border: "none", background: isHov ? accent : `${accent}18`, color: isHov ? "#fff" : accent, fontSize: 13, fontWeight: 700, cursor: "pointer", transition: "all 0.2s", display: "flex", alignItems: "center", gap: 7, width: "fit-content" }} tabIndex={-1}>
                Open {title} →
              </button>
            </div>
          );
      ...
      <p style={{ marginTop: 48, fontSize: 11, color: C.s6, textAlign: "center" }}>No account required · your data never leaves your browser</p>
    </div>
  );
}
...
export default function App() {
  ...
  const tabBtn = t => ({ padding: "7px 14px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, whiteSpace: "nowrap", transition: "all 0.15s", outline: "none", background: activeTab === t ? C.emerald : hoveredTab === t ? C.s8 : "transparent", color: activeTab === t ? "#fff" : hoveredTab === t ? C.light : C.s5 });
  const modeBtn = (cur, val) => ({ padding: "7px 16px", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", border: "none", transition: "all 0.2s", outline: "none", background: cur === val ? C.emerald : hoveredChannel === val ? C.s7 : "transparent", color: cur === val ? "#fff" : C.s5 });
  const marketBtn = val => ({ padding: "7px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", border: "none", transition: "all 0.2s", outline: "none", background: marketMode === val ? (val === "us" ? C.blue : C.violet) : hoveredMarket === val ? C.s7 : "transparent", color: marketMode === val ? "#fff" : C.s5, display: "flex", alignItems: "center", gap: 6 });
  ...
  const sectionHeader = (num, accent, icon, label) => (
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 18 }}>
      <div style={{ width: 20, height: 20, borderRadius: 5, background: accent, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontSize: 9, fontWeight: 800, color: "#000" }}>{num}</span>
      </div>
      <div style={{ color: accent }}>{icon}</div>
      <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: accent }}>{label}</span>
    </div>
  );
  ...
  return (
    <>
        ...
        {activeTool !== "home" && (
          <>
            ...
            {activeTool === "calculator" && (
              <div style={{ background: C.s95, minHeight: "calc(100vh - 52px)", padding: "20px 16px", ...SANS, color: C.light }}>
                ...
                <div style={{ maxWidth: 1140, margin: "0 auto" }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", alignItems: "flex-start", justifyContent: "space-between", gap: 14, marginBottom: 20 }}>
                    <div>
                      <h1 style={{ fontSize: 20, fontWeight: 700, margin: "0 0 4px", color: C.light, letterSpacing: "-0.02em" }}>{channelMode === "amazon" ? "Amazon FBA Profit Calculator" : "DTC / Shopify Profit Calculator"}</h1>
                      <p style={{ fontSize: 12, color: C.s5, margin: 0 }}>Unit economics & multi-channel margin simulator</p>
                    </div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
                      <div style={{ display: "flex", background: C.s9, border: `1px solid ${C.s8}`, borderRadius: 10, padding: 3, gap: 2 }}>
                      ...
                      <button onClick={handleShare} onMouseEnter={() => setHoveredShare(true)} onMouseLeave={() => setHoveredShare(false)}
                        style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 9, fontSize: 12, fontWeight: 600, background: hoveredShare ? C.s7 : C.s9, border: `1px solid ${C.s8}`, color: C.s4, cursor: "pointer", outline: "none", transition: "all 0.15s" }}>
                  ...
                  {/* Mode banner */}
                  <div style={{ marginBottom: 20, padding: "10px 14px", background: isUS ? C.blueDim : C.violetDim, border: `1px solid ${isUS ? C.blue+"30" : C.violet+"30"}`, borderRadius: 9, fontSize: 11, color: isUS ? "#93c5fd" : "#c4b5fd", display: "flex", gap: 8, alignItems: "center" }}>
                    {isUS ? <Flag size={12} /> : <Globe size={12} />}
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 18, alignItems: "flex-start" }}>
                    ...
                    <div style={{ flex: "1 1 260px", maxWidth: 310, minWidth: 250, display: "flex", flexDirection: "column", gap: 14 }}>
                      <div style={{ padding: "11px 14px", background: C.emeraldDim, border: `1px solid ${C.emerald}28`, borderRadius: 10, fontSize: 11, color: "#6ee7b7", lineHeight: 1.55, display: "flex", gap: 8, alignItems: "flex-start" }}>
                        <span style={{ fontSize: 13, flexShrink: 0 }}>👋</span>
                        <span>These are <strong>example values</strong>. Replace with your real product numbers.</span>
                      </div>

                      <div style={CARD}>
                        {sectionHeader(1, C.emerald, <Package size={13} />, "Product & Tax")}
                        <InputField label="Selling price (gross)" name="sellingPrice" value={inputs.sellingPrice} onChange={handleChange} prefix="$" highlight={C.emerald} tooltip="The price customers pay on Amazon. For EU, include VAT." error={inputErrors.sellingPrice} />
                      ...
                      <div style={CARD}>
                        {sectionHeader(2, C.cyan, channelMode === "amazon" ? <BarChart3 size={13} /> : <ShieldCheck size={13} />, channelMode === "amazon" ? "Amazon Fees" : "DTC Logistics")}
                        {channelMode === "amazon" ? (
                          <>
                      ...
                      <div style={CARD}>
                        {sectionHeader(3, C.orange, <TrendingUp size={13} />, "Marketing & Scale")}
                    ...
                    <div style={{ flex: "1 1 500px", minWidth: 300, display: "flex", flexDirection: "column", gap: 14 }}>
                      {(Number(inputs.sellingPrice) === 0 || Number(inputs.unitCost) === 0) && (
                        <div style={{ display: "flex", alignItems: "flex-start", gap: 8, background: C.amberDim, border: `1px solid ${C.amber}30`, borderRadius: 10, padding: "11px 14px", fontSize: 12, color: C.amber }}>
                      ...
                      {/* Tab bar */}
                      <div style={{ display: "flex", gap: 4, background: C.s9, border: `1px solid ${C.s8}`, borderRadius: 11, padding: "3px", overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
                      ...
                      {activeTab === "breakdown" && (
                        <div style={CARD}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                            <span style={{ fontSize: 13, fontWeight: 600, color: C.light }}>Unit P&L waterfall</span>
                            <span style={{ fontSize: 11, color: C.s5, display: "flex", alignItems: "center", gap: 4 }}><Info size={11} />{isUS ? "US: pre-tax" : "Intl: post-VAT"}</span>
                          </div>
                          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                            ...
                            <span style={{ fontSize: 9, color: C.s6, textTransform: "uppercase", letterSpacing: "0.08em", width: 40, textAlign: "right" }}>% price</span>
                          </div>
                          <WaterfallBar label="Gross selling price" value={s.sellingPrice} total={s.sellingPrice} color={C.cyan} />
                          {!isUS && <WaterfallBar label={`VAT extracted (${effectiveVat}%)`} value={-s.vatAmount} total={s.sellingPrice} color={C.s6} note="Remitted to tax authority — not your revenue" />}
                          {isUS && <WaterfallBar label="Sales tax (at checkout)" value={0} total={s.sellingPrice} color={C.s6} note="Collected by marketplace — excluded from unit P&L" />}
                          ...
                          <WaterfallBar label="Channel fees" value={-s.channelFees} total={s.sellingPrice} color={C.rose + "88"} note={!isUS && channelMode === "amazon" ? `Referral on gross $${fmt(s.sellingPrice)} → $${fmt(s.referralActual)}` : undefined} />
                          ...
                          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px,1fr))", gap: 12, marginTop: 20 }}>
                            <div style={CARD_INSET}>
                              ...
                              <div style={{ ...ROW, borderBottom: "none" }}><span style={{ fontSize: 12, color: C.light, fontWeight: 600 }}>Total landed</span><span style={{ fontSize: 12, fontWeight: 700, color: C.amber, ...MONO }}>${fmt(s.landedCost)}</span></div>
                            </div>
                            <div style={CARD_INSET}>
                              <div style={LABEL}>{channelMode === "amazon" ? "Amazon fee detail" : "DTC cost detail"}</div>
                              {channelMode === "amazon" ? (
                                <>
                                  <div style={ROW}><span style={{ fontSize: 12, color: C.s4 }}>Referral ({inputs.referralFee}%{!isUS?" of gross":""})</span><span style={{ fontSize: 12, fontWeight: 600, ...MONO }}>${fmt(s.referralActual)}</span></div>
                              ...
                              <div style={{ ...ROW, borderTop: `1px solid ${C.s8}`, marginTop: 6 }}><span style={{ fontSize: 12, color: C.light, fontWeight: 600 }}>Total fees</span><span style={{ fontSize: 12, fontWeight: 700, color: C.rose, ...MONO }}>${fmt(s.channelFees)}</span></div>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      {activeTab === "efficiency" && (
                        <div style={CARD}>
                          <div style={{ fontSize: 13, fontWeight: 600, color: C.light, marginBottom: 20 }}>Efficiency metrics & benchmarks</div>
                          <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                            {[
                              { label: "Break-even ACOS", val: isFinite(s.breakEvenAcos)?s.breakEvenAcos:0, max: 60, color: isFinite(s.breakEvenAcos)&&inputs.adSpendShare<=s.breakEvenAcos?C.emerald:C.rose, note: isFinite(s.breakEvenAcos)?`Ads must convert below ${fmt(s.breakEvenAcos,1)}% ACOS to be profitable`:"Price is $0 — cannot calculate" },
                              { label: "Current TACOS", val: inputs.adSpendShare, max: 60, color: inputs.adSpendShare<15?C.emerald:inputs.adSpendShare<25?C.amber:C.rose, note: inputs.adSpendShare<15?"Efficient":inputs.adSpendShare<25?"Moderate — target <15%":"High — reduce ad dependency" },
                              { label: "Net margin", val: isFinite(s.netMargin)?s.netMargin:0, max: 50, color: isFinite(s.netMargin)&&s.netMargin>20?C.emerald:isFinite(s.netMargin)&&s.netMargin>10?C.amber:C.rose, note: !isFinite(s.netMargin)?"Enter a selling price":s.netMargin>20?"Scale-ready":s.netMargin>10?"Viable — aim for 20%+":"Below minimum viable threshold" },
                              { label: "ROI on landed cost", val: isFinite(s.roi)?Math.max(0,s.roi):0, max: 200, color: isFinite(s.roi)&&s.roi>80?C.emerald:isFinite(s.roi)&&s.roi>40?C.amber:C.rose, note: !isFinite(s.roi)?"Enter a unit cost":`${s.roi>80?"Exceptional":s.roi>40?"Good":"Low"} — ${fmt(s.roi,0)}% return on capital` },
                              ...
                              <div key={label}>
                                ...
                                <p style={{ fontSize: 11, color: C.s5, margin: "5px 0 0" }}>{note}</p>
                              </div>
                            ))}
                          </div>
                          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: 10, paddingTop: 18, marginTop: 6, borderTop: `1px solid ${C.s8}` }}>
                            {[
                              ...
                              <div key={label} style={{ ...CARD_INSET, textAlign: "center" }}>
                                ...
                                <div style={{ fontSize: 10, color: C.s5, marginTop: 3 }}>{sub}</div>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {activeTab === "cashflow" && (
                        <div style={CARD}>
                          <div style={{ fontSize: 13, fontWeight: 600, color: C.light, marginBottom: 20 }}>Inventory cash flow & payback</div>
                          <div style={{ marginBottom: 20 }}>
                            ...
                            <input type="number" value={orderQty} min={1} onChange={e => setOrderQty(Math.max(1, parseInt(e.target.value)||1))}
                              style={{ width: 160, background: C.s95, border: `1px solid ${C.s7}`, borderRadius: 8, padding: "8px 12px", fontSize: 13, color: C.light, outline: "none", ...MONO }} />
                          </div>
                          {(() => {
                            ...
                            return (
                              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
                                {[
                                  { label: "Capital at risk", value: `$${fmtK(capitalAtRisk)}`, sub: `${orderQty} units × $${fmt(s.totalCOGS)} COGS`, color: C.rose },
                                  { label: "Payback period", value: paybackMonths!=null?`${fmt(paybackMonths,1)} mo`:"Never", sub: paybackMonths!=null?`at $${fmtK(monthlyProfit)}/mo profit`:"Unprofitable", color: paybackMonths!=null&&paybackMonths<=3?C.emerald:paybackMonths!=null&&paybackMonths<=6?C.amber:C.rose },
                          ...
                          <div style={{ marginTop: 20, ...CARD_INSET, fontSize: 12, color: C.s4, lineHeight: 1.65 }}>
                            <strong style={{ color: C.light }}>Rule of thumb:</strong> Payback under 3 months = aggressive scaling candidate. 3–6 months = healthy. Over 6 months = cash flow risk.
                          </div>
                        </div>
                      )}
                      ...
                      {activeTab === "pricing" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                          <div style={CARD}>
                            <div style={{ fontSize: 13, fontWeight: 600, color: C.light, marginBottom: 20 }}>Reverse calculator — solve for target margin</div>
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 14, alignItems: "flex-end", marginBottom: 24 }}>
                              <div>
                                ...
                                <input type="number" value={targetMargin} min={1} max={99} onChange={e => setTargetMargin(Math.min(99,Math.max(1,parseFloat(e.target.value)||20)))}
                                  style={{ width: 100, background: C.s95, border: `1px solid ${C.emerald}`, borderRadius: 8, padding: "8px 12px", fontSize: 13, color: C.light, outline: "none", ...MONO }} />
                              </div>
                              <div style={{ display: "flex", gap: 3, background: C.s95, border: `1px solid ${C.s8}`, borderRadius: 10, padding: 3 }}>
                                {[["price","Find min price"],["cogs","Find max COGS"]].map(([val,label]) => (
                                  <button key={val} onClick={() => setSolveFor(val)} style={{ padding: "7px 14px", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: "pointer", border: "none", background: solveFor===val?C.emerald:"transparent", color: solveFor===val?"#fff":C.s5, transition: "all 0.15s" }}>{label}</button>
                                ))}
                              </div>
                            </div>
                            {(() => {
                              ...
                              return feasible ? (
                                <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                                  <div style={{ ...CARD_INSET, flex: 1, minWidth: 160 }}>
                                    ...
                                    <div style={{ fontSize: 28, fontWeight: 700, color: C.emerald, ...MONO }}>${fmt(solveFor==="price"?solvedPrice:solvedCOGS)}</div>
                                  ...
                                  <div style={{ ...CARD_INSET, flex: 1, minWidth: 160 }}>
                                    ...
                                    <div style={{ fontSize: 28, fontWeight: 700, color: gap!=null&&gap>=0?C.emerald:C.rose, ...MONO }}>
                                      {gap!=null?`${gap>=0?"+":""}$${fmt(Math.abs(gap))}`:"—"}
                                    </div>
                                    <div style={{ fontSize: 11, color: C.s5, marginTop: 4 }}>{gap!=null&&gap>=0?(solveFor==="price"?"current price exceeds target ✓":"room to negotiate up ✓"):(solveFor==="price"?"need to raise price":"must reduce COGS")}</div>
                                  </div>
                                </div>
                              ) : (
                                <div style={{ padding: "14px 18px", background: C.roseDim, border: `1px solid ${C.rose}30`, borderRadius: 10, color: C.rose, fontSize: 13 }}>
                          ...
                          <div style={CARD}>
                            <div style={{ fontSize: 13, fontWeight: 600, color: C.light, marginBottom: 16 }}>Price sensitivity</div>
                            <div style={{ borderRadius: 10, border: `1px solid ${C.s8}`, overflow: "hidden" }}>
                              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                                <thead>
                                  <tr style={{ background: C.s95 }}>
                                    {["Price","Net profit/unit","Margin %","Monthly profit","ROI"].map(h => (
                                      <th key={h} style={{ padding: "9px 12px", textAlign: "right", color: C.s5, fontWeight: 700, fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em", borderBottom: `1px solid ${C.s8}` }}>{h}</th>
                                    ))}
                                  </tr>
                                </thead>
                                <tbody>
                                  {[-10,-5,-2,-1,0,1,2,5,10].map(delta => {
                                    ...
                                    return (
                                      <tr key={delta} style={{ borderBottom: `1px solid ${C.s85}`, background: isCurrent?`${C.emerald}0c`:"transparent" }}>
                                        <td style={{ padding: "9px 12px", textAlign: "right", fontWeight: isCurrent?700:400, color: isCurrent?C.emerald:C.light, ...MONO }}>${fmt(P)}{isCurrent&&<span style={{ fontSize: 9, color: C.s5, marginLeft: 4 }}>current</span>}</td>
                                        <td style={{ padding: "9px 12px", textAlign: "right", color: profit>=0?C.emerald:C.rose, ...MONO, fontWeight: 600 }}>{profit>=0?`$${fmt(profit)}`:`-$${fmt(Math.abs(profit))}`}</td>
                                        <td style={{ padding: "9px 12px", textAlign: "right", color: margin>20?C.emerald:margin>10?C.amber:C.rose, ...MONO }}>{fmt(margin,1)}%</td>
                                        <td style={{ padding: "9px 12px", textAlign: "right", color: monthlyProfit>=0?C.light:C.rose, ...MONO }}>{monthlyProfit>=0?`$${fmtK(monthlyProfit)}`:`-$${fmtK(Math.abs(monthlyProfit))}`}</td>
                                        <td style={{ padding: "9px 12px", textAlign: "right", color: roi>50?C.emerald:roi>20?C.amber:C.rose, ...MONO }}>{fmt(roi,0)}%</td>
                                      </tr>
                                    );
                                  })}
                                </tbody>
                      ...
                      {activeTab === "insights" && (
                        <div style={CARD}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
                            ...
                            <span style={{ fontSize: 13, fontWeight: 600, color: C.light }}>AI-driven strategic insights</span>
                            <span style={{ marginLeft: "auto", fontSize: 10, color: C.s5, background: C.s8, borderRadius: 6, padding: "3px 8px", border: `1px solid ${C.s7}` }}>Live · {insights.length} signal{insights.length!==1?"s":""}</span>
                          </div>
                          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                            {insights.map((ins, i) => <InsightCard key={i} {...ins} />)}
                          </div>
                          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 10, marginTop: 16 }}>
                            <div style={{ ...CARD_INSET, borderColor: `${C.emerald}22` }}>
                              ...
                              <p style={{ fontSize: 12, color: C.s4, margin: 0, lineHeight: 1.65 }}>
            ...
            {activeTool === "ppc" && (
              <div style={{ background: C.s95, minHeight: "calc(100vh - 52px)", padding: "20px 16px", ...SANS }}>
                <div style={{ maxWidth: 1140, margin: "0 auto" }}>
      ...
      {toast && (
        <div style={{ position: "fixed", bottom: 28, left: "50%", transform: "translateX(-50%)", background: C.emerald, color: "#fff", padding: "10px 22px", borderRadius: 10, fontSize: 13, fontWeight: 600, zIndex: 100, boxShadow: "0 4px 24px #10b98150" }}>{toast}</div>
      )}

      <style>{`
        * { box-sizing: border-box; }
        body { margin: 0; background: ${C.s95}; }
        ...
        ::-webkit-scrollbar-track { background: ${C.s9}; }
        ::-webkit-scrollbar-thumb { background: ${C.s7}; border-radius: 3px; }
        ::-webkit-scrollbar-thumb:hover { background: ${C.s6}; }
      `}</style>
      <Analytics />
    </>
  );
